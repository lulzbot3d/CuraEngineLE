...
class CuraEngineLEConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.options.with_cura_resources:
            for req in self.conan_data["requirements_curale_resources"]:
                self.requires(req)
        if self.options.enable_arcus or self.options.enable_plugins:
        ...
        if self.options.get_safe("enable_sentry", False):
            ...
            if which("sentry-cli") is None:
                ...
                if self.settings.os == "Linux":
                    ...
                    self.run("dsymutil CuraEngineLE")
                ...
                self.run(f"sentry-cli --auth-token {os.environ['SENTRY_TOKEN']} releases set-commits -o {sentry_org} -p {sentry_project} --commit \"lulzbot3d/CuraEngineLE@{self.conan_data['commit']}\" {self.version}")
